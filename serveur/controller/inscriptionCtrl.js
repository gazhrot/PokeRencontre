				...
				if (rows.length < 1) {
					var aquery = connection.query('INSERT INTO user (email, password) VALUES (?, ?)', [req.body.email, req.body.password], function(err, rows) {
						if (err) {
							...
							res.send(JSON.stringify({error: true, data: 'inscription reussie', token: ''}));
						}
					});
				} else {
					res.send(JSON.stringify({error: false, data: 'Email ou Password invalide, ou Email deja utiliser.'}));
				}
			}
		});
	}
